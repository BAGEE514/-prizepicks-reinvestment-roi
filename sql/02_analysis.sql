---Analysis1: Score every player on Recency, Frequency, Monetary value, then bucket into quintiles
...
WITH activity AS (
 ...
 FROM players p LEFT JOIN entries e ON e.player_id = p.player_id
...
SELECT player_id, value_tier, freq, monetary, r_score, f_score, m_score,
...
FROM scored ORDER BY rfm_total DESC;

---Analysis2: Group players by sign up weekand measure what share were still entering contests in later weeks 

WITH base AS (
 SELECT player_id, strftime('%Y-%W', signup_date) AS cohort_week, signup_date
 FROM players
),
acts AS (
 ...
 FROM base b JOIN entries e ON e.player_id = b.player_id
)
SELECT cohort_week,
...
FROM acts GROUP BY cohort_week ORDER BY cohort_week;

---Analysis3: A windowed running sum of deposits per player plus LAG for deposit-to-deposit change. 

WITH dep AS (
 SELECT player_id, deposit_date, amount,
 SUM(amount) OVER (PARTITION BY player_id ORDER BY deposit_date
 ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_deposits,
...
FROM dep WHERE player_id IN (1,2,3)
ORDER BY player_id, deposit_seq;

---Analysis4: Revenue = entry fees collected minus payouts, rolled up by tier and channel — the ‘user trends → business KPI’ 


SELECT p.value_tier, p.acquisition_channel,
...
FROM players p JOIN entries e ON e.player_id = p.player_id
